...
- **Push the branch and open a GitHub PR against `main` before landing it**, so the `ci.yml` checks (backend tests, frontend build, Docker build) run and are visible on the PR — this is what branch protection on `main` gates the merge on. Wait for CI to go green before merging.
- **To land a branch, merge it into `main` with a fast-forward only merge** (`git checkout main && git merge --ff-only <branch>`), then push `main` and delete the branch (`git branch -d <branch>`) — GitHub auto-closes the PR as merged once it sees the ff-only merge commit land on `main`. If `--ff-only` fails because `main` has diverged, stop and ask — don't force it and don't fall back to a merge commit without checking with the user first.
